...
//Создаём переход на активность с результатом
        ...
        //Создаём временные переменные temp_1,2,3,4. Переменная temp_1 - принимает значение
        // правильной столицы (по индексу массива Столиц, т.к. индексы Стран совпадают с
        // правильными индексами Столиц), а для temp_2,3,4 мы генерируем псевдослучайное
...
// их поля в activity_tren_2 - android:text="Button", так как туда мы будем вставлять Столицу
...
//Устанавливаем первоначальное значение для кнопки 1
        ...
        // При этом мы не можем вывести
        // число в поле TextView, для этого необходимо число преобразовать в текст методом String s = Integer.toString(преобразовываемое число);



//39.Подключаем 4 временные переменные, для реакции на нажатие кнопки.
//Страна
...
//Мы реализуем интерфейс слушателя события. Собитие у нас - клик по кнопке. В круглых скобках "...stener(new View ... });" создаётся новый безымянный класс,
        //который будет реализован вызываемым методом onClick
        //То есть после клика, запускается выбор индекса
